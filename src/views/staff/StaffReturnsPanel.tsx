...
export default function StaffReturnsPanel({ user, returns, completeReturnInspection, canProcessReturns, showToast }: Props) {
  ...
  return <div className="fade-in space-y-5"><SectionHeader title={'Nghiệm Thu Trả Kho'} subtitle={'Biên bản, phí và bằng chứng được lưu vào dữ liệu dùng chung.'} /><Card><Table><Thead><tr><Th>{'Mã hồ sơ'}</Th><Th>{'Khách hàng'}</Th><Th>{'Gian kho'}</Th><Th>{'Ngày hẹn'}</Th><Th>{'Trạng thái'}</Th><Th /></tr></Thead><Tbody>{returns.map(item => <Tr key={item.id}><Td className="font-mono">{item.id}</Td><Td>{item.customerName}</Td><Td>{item.unitId}</Td><Td>{item.scheduledDate}</Td><Td><Badge variant={item.status === 'completed' ? 'success' : 'warning'}>{({ requested: 'Chờ tiếp nhận', scheduled: 'Đã lên lịch', inspected: 'Đã nghiệm thu', awaiting_customer_confirmation: 'Chờ khách hàng xác nhận', pending_refund: 'Chờ hoàn cọc', completed: 'Đã hoàn tất', disputed: 'Cần đối chiếu' } as Record<string, string>)[item.status] || 'Đang xử lý'}</Badge></Td><Td className="text-right">{item.status === 'requested' && canProcessReturns && <Button size="sm" onClick={() => setSelected(item)}>{'Nghiệm thu'}</Button>}</Td></Tr>)}</Tbody></Table>{!returns.length && <div className="p-10 text-center text-sm text-stone-500">{'Chưa có yêu cầu trả kho.'}</div>}</Card><Modal closeLabel="Đóng hộp thoại" open={Boolean(selected)} onClose={() => setSelected(null)} title={'Biên bản nghiệm thu'}><div className="space-y-4"><Input label={'Ghi chú hiện trạng'} value={notes} onChange={event => setNotes(event.target.value)} /><Input label={'Ảnh bằng chứng'} value={evidence} onChange={event => setEvidence(event.target.value)} /><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setSelected(null)}>{'Hủy'}</Button>{canProcessReturns && <Button disabled={!notes.trim() || !evidence.trim()} onClick={inspect}>{'Lưu biên bản'}</Button>}</div></div></Modal></div>
}

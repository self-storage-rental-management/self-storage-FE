...
export default function StaffReservationsPanel({ user, reservations, approveReservation, showToast }: Props) {
  ...
  const approve = (id: string) => {
    try {
      ...
      showToast('Đã phê duyệt hồ sơ. Customer có thể thanh toán cọc.')
    } catch (error) {
  ...
  return <div className="fade-in space-y-5">
    ...
    <Card>
      <Table><Thead><tr><Th>{'Mã'}</Th><Th>Customer</Th><Th>{'Cỡ kho'}</Th><Th>{'Ngày vào'}</Th><Th>{'Trạng thái'}</Th><Th /></tr></Thead>

...
export default function StaffApp({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  return <Layout user={user} navItems={nav} currentPage={page} onNavigate={setPage} onLogout={onLogout} canAccess={permission => hub.can(user, permission)} roleLabel={'Nhân Viên Cơ Sở'} roleColor="bg-green-100 text-green-700">
    ...
    {page === 'checkin' && <StaffCheckinsPanel user={user} checkins={checkins} holds={hub.holds} signPaperContract={hub.signPaperContract} payRemainingBalance={hub.payRemainingBalance} completeCheckIn={hub.completeCheckIn} canPerformCheckin={hub.can(user, 'perform_checkin')} canManagePayments={hub.can(user, 'manage_payments')} showToast={showToast} />}
    {page === 'return' && <StaffReturnsPanel user={user} returns={returns} completeReturnInspection={hub.completeReturnInspection} canProcessReturns={hub.can(user, 'process_returns')} showToast={showToast} />}
    {page === 'support' && <StaffSupportPanel user={user} tickets={tickets} respondSupportTicket={hub.respondSupportTicket} canManageSupport={hub.can(user, 'manage_support')} showToast={showToast} />}

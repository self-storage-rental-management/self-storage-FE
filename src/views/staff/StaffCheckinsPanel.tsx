...
export default function StaffCheckinsPanel({ user, checkins, holds, signPaperContract, payRemainingBalance, completeCheckIn, canPerformCheckin, canManagePayments, showToast }: Props) {
  ...
  const prepare = () => {
    ...
    try {
      ...
      setSelected(null)
      showToast('Đã lưu hợp đồng và thanh toán còn lại. Hồ sơ sẵn sàng Nhận kho.')
    } catch (error) {
  ...
  const finish = () => {
    ...
    try {
      ...
      setSelected(null)
      showToast('Nhận kho hoàn tất; hợp đồng thuê đã được kích hoạt trong dữ liệu dùng chung.')
    } catch (error) {
  ...
  return <div className="fade-in space-y-5"><SectionHeader title={'Nhận kho & Bàn Giao'} subtitle={'Chỉ hồ sơ đã cọc, phân kho và đủ điều kiện mới được bàn giao.'} /><Card><Table><Thead><tr><Th>{'Lịch'}</Th><Th>{'Khách hàng'}</Th><Th>{'Gian kho'}</Th><Th>{'Trạng thái'}</Th><Th /></tr></Thead><Tbody>{checkins.map(item => <Tr key={item.id}><Td>{item.scheduledDate}<br /><span className="text-xs text-stone-500">{item.scheduledTime}</span></Td><Td>{item.customerName}</Td><Td>{item.unitId}</Td><Td><Badge variant={item.status === 'completed' ? 'success' : item.status === 'cancelled' ? 'error' : 'info'}>{({ scheduled: 'Đã lên lịch', completed: 'Đã hoàn tất', cancelled: 'Đã hủy' } as Record<string, string>)[item.status] || 'Chờ xử lý'}</Badge></Td><Td className="text-right">{item.status === 'scheduled' && (canPerformCheckin || canManagePayments) && <Button size="sm" onClick={() => setSelected(item)}>{'Thực hiện'}</Button>}</Td></Tr>)}</Tbody></Table>{!checkins.length && <div className="p-10 text-center text-sm text-stone-500">{'Chưa có lịch Nhận kho.'}</div>}</Card><Modal closeLabel="Đóng hộp thoại" open={Boolean(selected)} onClose={() => setSelected(null)} title={'Hoàn tất Nhận kho'}><div className="space-y-4"><Input label={'Tình trạng ban đầu'} value={condition} onChange={event => setCondition(event.target.value)} /><Input label={'Ảnh/biên bản bằng chứng'} value={evidence} onChange={event => setEvidence(event.target.value)} /><div className="rounded-lg border border-stone-200 bg-stone-50 p-3 text-xs text-stone-600">{ready ? ('Hồ sơ đã đủ hợp đồng, thanh toán và phân kho.') : ('Bước đầu: ghi nhận hợp đồng giấy và khoản còn lại tại quầy.')}</div><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setSelected(null)}>{'Hủy'}</Button>{ready ? (canPerformCheckin && <Button disabled={!condition.trim() || !evidence.trim()} onClick={finish}>{'Xác nhận bàn giao'}</Button>) : (canPerformCheckin && canManagePayments && <Button disabled={!evidence.trim()} onClick={prepare}>{'Hoàn tất hồ sơ tại quầy'}</Button>)}</div></div></Modal></div>
}

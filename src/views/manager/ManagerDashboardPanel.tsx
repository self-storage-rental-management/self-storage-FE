...
export default function ManagerDashboardPanel({ user, setPage }: Props) {
  ...
  const pendingReview = facilityHolds.filter(item => item.status === 'awaiting_review' && item.goodsReviewStatus === 'PENDING')
  ...
  const queues = [
    { label: 'Hồ sơ chờ duyệt hàng hóa', count: pendingReview.length, page: 'reservations', variant: pendingReview.length ? 'info' : 'success' },
    { label: 'Đơn đã cọc chờ phân kho', count: unassigned.length, page: 'reservations', variant: unassigned.length ? 'warning' : 'success' },
    { label: 'Lịch nhận kho sắp tới', count: upcomingMoves.length, page: 'moves', variant: 'info' },
    { label: 'Hồ sơ trả kho đang mở', count: openReturns.length, page: 'moves', variant: openReturns.some(item => item.status === 'disputed') ? 'error' : 'warning' },
  ...
  return <div className="fade-in space-y-5">
    <SectionHeader eyebrow={'Tổng quan cơ sở'} title="Bảng điều khiển" subtitle={user.facility || ('Chưa gán cơ sở')} action={<Button variant="outline" onClick={() => setPage('reports')}>{'Mở báo cáo'}</Button>} />
    ...
    <Card className="p-5"><div className="mb-2 flex items-center justify-between"><h3 className="font-bold">{'Công suất cơ sở'}</h3><b>{occupied}/{facilityUnits.length}</b></div><ProgressBar value={occupied} max={Math.max(1, facilityUnits.length)} /><div className="mt-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4"><span>Còn trống: <b>{facilityUnits.filter(item => item.status === 'available').length}</b></span><span>Đã giữ chỗ: <b>{facilityUnits.filter(item => item.status === 'reserved').length}</b></span><span>Đang sử dụng: <b>{occupied}</b></span><span>Bảo trì: <b>{facilityUnits.filter(item => item.status === 'maintenance').length}</b></span></div></Card>
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{queues.map(item => <Card key={item.page + item.label} className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold text-stone-900">{item.label}</p><p className="mt-1 text-2xl font-extrabold">{item.count}</p></div><Badge variant={item.variant}>{item.count ? ('Cần xử lý') : 'Ổn định'}</Badge></div><Button className="mt-4 w-full" size="sm" variant="outline" onClick={() => setPage(item.page)}>{'Mở hàng đợi'}</Button></Card>)}</div>
  </div>
}

...
export default function ManagerPaymentsPanel({ user, rentals, payments, config, recordRentalPayment, applyRentalLateFee, waiveRentalLateFee, setRentalOverlock, sendDelinquencyReminder, showToast }: Props) {
  ...
  return <div className="fade-in space-y-5">
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><StatCard title={lang === 'vi' ? 'Đã thu' : 'Collected'} value={`$${collected.toFixed(2)}`} icon={Icon.dollar} /><StatCard title={lang === 'vi' ? 'Đang quá hạn' : 'Overdue accounts'} value={overdueRentals.length} icon={Icon.alert} /><StatCard title={lang === 'vi' ? 'Tổng cần thu' : 'Outstanding'} value={`$${outstanding.toFixed(2)}`} icon={Icon.credit} /><StatCard title={lang === 'vi' ? 'Đang khóa truy cập' : 'Access suspended'} value={overdueRentals.filter(rental => rental.overlocked).length} icon={Icon.key} /></div>

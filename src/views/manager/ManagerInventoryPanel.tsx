...
export default function ManagerInventoryPanel({ user, units, maintenanceTasks, updateUnitStatus, showToast }: Props) {
  ...
  const saveStatus = () => {
    ...
    try {
      ...
      showToast(lang === 'vi' ? `Đã chuyển ${selectedUnit.code} sang ${targetStatus.toUpperCase()}.` : `${selectedUnit.code} moved to ${targetStatus.toUpperCase()}.`)
  ...
  return <div className="fade-in space-y-5">
    <SectionHeader eyebrow={lang === 'vi' ? 'Tồn kho vật lý' : 'Physical inventory'} title="Storage Inventory" subtitle={lang === 'vi' ? 'Theo dõi trạng thái khai thác, thông số và hàng đợi bảo trì của cơ sở.' : 'Facility-scoped unit availability, specifications and maintenance queue.'} />

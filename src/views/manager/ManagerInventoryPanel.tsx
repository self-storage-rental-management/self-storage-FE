...
export default function ManagerInventoryPanel({ user, units, maintenanceTasks, updateUnitStatus, showToast }: Props) {
  ...
  const saveStatus = () => {
    ...
    try {
      ...
      showToast(`Đã chuyển ${selectedUnit.code} sang ${managerStatusLabel(targetStatus, 'vi')}.`)
  ...
  return <div className="fade-in space-y-5">
    <SectionHeader eyebrow={'Tồn kho vật lý'} title="Tồn kho gian kho" subtitle={'Theo dõi trạng thái khai thác, thông số và hàng đợi bảo trì của cơ sở.'} />
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      <StatCard title={'Tổng gian'} value={facilityUnits.length} icon={Icon.box} />
      <StatCard title={'Còn trống'} value={facilityUnits.filter(unit => unit.status === 'available').length} icon={Icon.check} />

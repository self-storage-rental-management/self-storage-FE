...
export default function ManagerStaffTasksPanel({ user, facilityId, facilityName, tasks, createFacilityTask, updateFacilityTask, showToast }: Props) {
  ...
  return <div className="fade-in space-y-5">
    <SectionHeader eyebrow={'Điều phối ca làm việc'} title="Staff & Tasks" subtitle={'Tạo, phân công và theo dõi nhiệm vụ vận hành của cơ sở.'} action={<Button onClick={() => setModalOpen(true)}>{'Tạo nhiệm vụ'}</Button>} />
    ...
    <div className="grid gap-4 lg:grid-cols-3">{staff.map(member => <Card key={member.id} className="p-4"><div className="flex items-start gap-3"><Avatar name={member.name} size="lg" /><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><p className="truncate font-bold text-stone-900">{member.name}</p><Badge variant={member.status === 'on-duty' ? 'success' : 'muted'}>{member.status}</Badge></div><p className="text-xs text-stone-500">{member.role} · {'shift' in member && typeof member.shift === 'string' ? member.shift : '—'}</p><p className="mt-2 text-xs text-stone-600">{member.email} · {member.phone}</p><p className="mt-2 font-semibold text-amber-800">{facilityTasks.filter(task => task.assignedStaffId === member.id && task.status !== 'completed').length} {'nhiệm vụ đang mở'}</p></div></div></Card>)}</div>
    ...
    <Card><Table><Thead><tr><Th>{'Nhiệm vụ'}</Th><Th>{'Loại'}</Th><Th>{'Người phụ trách'}</Th><Th>{'Hạn xử lý'}</Th><Th>{'Ưu tiên'}</Th><Th>{'Trạng thái'}</Th><Th /></tr></Thead><Tbody>
      {facilityTasks.map(task => <Tr key={task.id}><Td><p className="font-semibold text-stone-900">{task.title}</p><p className="text-xs text-stone-400">{task.id}{task.referenceId ? ` · ${task.referenceId}` : ''}</p>{task.notes && <p className="mt-1 max-w-md text-xs text-stone-500">{task.notes}</p>}</Td><Td>{task.type}</Td><Td><Select value={task.assignedStaffId || ''} onChange={event => { const selected = staff.find(item => item.id === event.target.value); run(() => updateFacilityTask(task.id, { assignedStaffId: selected?.id, assignedStaffName: selected?.name, status: selected ? 'in_progress' : 'open' }, user), 'Đã cập nhật người phụ trách.') }}><option value="">{'Chưa phân công'}</option>{staff.map(member => <option key={member.id} value={member.id}>{member.name}</option>)}</Select></Td><Td>{task.dueAt}</Td><Td><Badge variant={task.priority === 'high' ? 'error' : task.priority === 'medium' ? 'warning' : 'info'}>{task.priority}</Badge></Td><Td><Badge variant={task.status === 'completed' ? 'success' : task.status === 'in_progress' ? 'info' : 'warning'}>{task.status}</Badge></Td><Td className="text-right">{task.status !== 'completed' && <Button size="sm" variant="outline" onClick={() => run(() => updateFacilityTask(task.id, { status: 'completed' }, user), 'Đã hoàn tất nhiệm vụ.')}>{'Hoàn tất'}</Button>}</Td></Tr>)}
